...
- **Custom** -> Ermöglicht das Erstellen eines eigenen Authentifizierungsschemas von Grund auf. Damit haben Sie vollständige Kontrolle über die Authentifizierungslogik.
...
- **Internal Application Extension** -> Verwenden Sie dieses Schema für Erweiterungen interner Anwendungen.
...
!!! exercise "Eigene Authentifizierung bauen"
    ...
    Zuerst fügen wir der Tabelle `EMP` eine Passwortspalte hinzu und füllen sie mit `ENAME` in Kleinbuchstaben. Führen Sie die folgenden Statements in **SQL Commands** im **SQL Workshop** aus, jeweils nacheinander. Mehrere Statements auf einmal können in **SQL Scripts** ausgeführt werden.
    ...
    Es gibt bereits ein Schema, das wir übrigens im Wizard beim Erzeugen der Anwendung ausgewählt haben. Klicken Sie auf **Create**.

    ![createscheme](assets/authentication/createscheme.png){ style="display:block;margin:auto;" }

    Wählen Sie **Based on a pre-configured scheme from the gallery** und klicken Sie auf **Next**.
...
Um das neue Schema zu testen, müssen Sie zuerst die aktuelle Anwendungssitzung beenden. Das geht auf jeder Seite der laufenden Anwendung oben rechts, wo der aktuell angemeldete Benutzer angezeigt wird. Danach können Sie sich mit einem beliebigen Mitarbeiternamen aus der Tabelle `EMP` anmelden. Als Passwort verwenden Sie den Namen in Kleinbuchstaben. Anschliessend sehen Sie den angemeldeten Benutzer oben rechts in der Anwendung.
