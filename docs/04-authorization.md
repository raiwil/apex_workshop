...
!!! bytheway "Referencing Bind Variables"
    *By the way*,<br>
    Bind-Variables like APP_USER or Page items can be integrated in different ways at various points throughout the application.:
    - **:MYVARIABLE** – with a colon inside APEX in SQL & PL/SQL as seen in this example.
    - **v('MYVARIABLE')** – with the function v and nv (for values and numeric value) in PL/SQL Code outside of APEX 
    - **&MYVARIABLE.**  – enclosed by an ampersand and a dot (don’t forget this dot at the end) as a substitution string like in APEX Items (Static Text)
