...
- **Database Accounts** -> This authentication scheme requires that a database user (schema) exists in the local database. When using this method, the user name and password of the database account are used to authenticate the user.
...
So far, we have authenticated the application users against users defined locally in APEX. This is not normally done, except perhaps during development. We now create our own authentication and simply define the employees of our table EMP as users of the application. Rather carelessly, we store the password in plain text in the table.
...
!!! exercise "Build Custom Authentication"
    ...
    First, we add a password column to the table EMP and fill it with the ENAME in lowercase. Run the following statements in **SQL Commands** in **SQL Workshop** (one after the other; running more than one statement at once can be done in **SQL Scripts**).
    ...
    Give the scheme a **Name** (here `MyEmpAuth`) with **Scheme Type** `Custom`. The **Authentication Function Name** will be `myauth`, and we define this function in the **PL/SQL Code** editor.
    ...
    The new scheme is now available, but it is not the active scheme (**Is Current**). Click the name of the new scheme.

    ![iscurrent](assets/authentication/iscurrent.png){ style="display:block;margin:auto;" }
    
    Now you can click **Make Current Scheme** to define your new scheme as the active scheme for the application.

    ![makecurrent](assets/authentication/makecurrent.png){ style="display:block;margin:auto;" }    

To test the new scheme, you first have to disconnect your current application session. You can do this on any page of the running application at the top right, where you see the currently connected user. Now you can log in with any of the employee names in the EMP table (using the name in lowercase as the password), and you will see the connected user at the top right of the application.
